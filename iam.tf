...
resource "aws_iam_role" "sa" {
  name = "${var.env}-${var.project_name}-parameter-store-eks-sa"
  assume_role_policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
        ...
        "Condition" : {
          "StringLike" : {
            ...
            "oidc.eks.us-east-1.amazonaws.com/id/${split("/", aws_eks_cluster.main.identity[0].oidc[0].issuer)[4]}:sub" : "system:serviceaccount:default:*"
          }
        }
      }
    ]
